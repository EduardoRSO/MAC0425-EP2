...
class ProgressionPlanning(object):
    ...
    def applicable(self, state):
        ...
        possible_actions = []
        for acao in all_actions:
            if set(precond(acao)).issubset(state):
                possible_actions = possible_actions + [acao]
    ...
    def successor(self, state, action):
        ...
        sucessor = []
        neg = neg_effect(action)
        pos = pos_effect(action)

        for acao in state:
            if acao not in neg:
                sucessor = sucessor + [acao]

        for acao in state:
            if acao not in sucessor:
                sucessor = sucessor + [acao]
    ...
    def solve(self, W, heuristics):
        ...
        fronteira = bla
        fronteira.push(self.problem.init())

        while not fronteira.is_empty():
            sN = fronteira.pop()
            if not sN in visited:
                visited.append(sN):
                    if goal_test(????):
                        return tripla
                for naosei in sucessor
                    tripla adiciona
                    fronteira.push(algo)
